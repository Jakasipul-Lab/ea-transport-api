...
export default function SafarioutesPage() {
  ...
  const handleSearch = async (e) => {
    ...
    // FIX 1: YOUR REAL BACKEND URL
    const searchUrl = query.trim()
      ? `https://ea-transport-api.onrender.com/api/search?q=${encodeURIComponent(query.trim())}`
      : `https://ea-transport-api.onrender.com/api/search`
    ...
    <main className="container mx-auto px-4 py-12 max-w-4xl">
                ...
                <CardContent className="pt-6 flex justify-between items-center">
                  <div>
                    <span className="text-xs font-semibold uppercase px-2 py-1 bg-emerald-100 text-emerald-800 rounded">
                      {route.category}  {/* FIX 2: was route.operator */}
                    </span>

                    <h3 className="text-lg font-bold mt-2 flex items-center gap-2">
                      <MapPin className="w-4 h-4 text-emerald-600" />
                      {route.name}  {/* FIX 3: was route.origin → route.destination */}
                    </h3>

                    <p className="text-sm text-muted-foreground flex items-center gap-1 mt-1">
                      <Clock className="w-3 h-3" />
                      Price: {route.price} KES {/* FIX 4: was route.time */}
                    </p>
                  </div>
                </CardContent>
              </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-3 text-sm">
          <p>• <strong>Masai Mara Circuit:</strong> 4x4 safari transfers and air charters from Nairobi.</p>
          <p>• <strong>Amboseli & Tsavo:</strong> Wildlife safari and lodge transfers.</p>
          <p>• <strong>Coastal Tourism:</strong> SGR services to Mombasa with Diani and Watamu connections.</p>
          <p>• <strong>Cross-Border Travel:</strong> Coaches to Arusha, Namanga, Kigali and Kampala.</p>
        </CardContent>
      </Card>
    </main>
  )

...
export default function JakasipulPage() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setHasSearched(true)
    try {
      // Connects directly to your Python FastAPI backend
  ...
  return (
    <main className="container mx-auto px-4 py-12 max-w-4xl">
      ...
      <p className="text-muted-foreground mb-6">Reliable local transport for your daily commute across East Africa.</p>

      {/* Search Bar connected to transport.db */}
      <form onSubmit={handleSearch} className="flex gap-2 mb-8">
        <div className="relative flex-1">
          <Search className="absolute left-3 top-3 w-5 h-5 text-muted-foreground" />
          <input
            type="text"
            placeholder="Search local matatu route (e.g. Rongai, Ngong, Kasarani)..."
            value={query}
      ...
      {/* Default Terminals Card */}
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Bus className="w-5 h-5" /> Major Local Terminals & Hubs
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          <p>• <strong>Nairobi:</strong> Githurai, Dandora, Rongai, Ngong, Kasarani, Westlands</p>
          <p>• <strong>Mombasa:</strong> Likoni, Nyali, Bamburi, Changamwe, Mtongwe</p>
